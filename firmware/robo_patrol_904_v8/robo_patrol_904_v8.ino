...
void loop()
{
  if (nh.connected())
  {
    ...
    if (t > (1000.0 / JOINTS_STATE_PUBLISH_RATE_HZ))
    {
      ...
      state_vel[0] = M1.impulse2rad(M1.rel_tiks * ((float)1000.0 / t));
      state_vel[1] = M2.impulse2rad(M2.rel_tiks * ((float)1000.0 / t));
      state_vel[2] = M3.impulse2rad(M3.rel_tiks * ((float)1000.0 / t));
      state_vel[3] = M4.impulse2rad(M4.rel_tiks * ((float)1000.0 / t));

      state_eff[0] = M1.getRPM();
      state_eff[1] = M2.getRPM();
      state_eff[2] = M3.getRPM();
